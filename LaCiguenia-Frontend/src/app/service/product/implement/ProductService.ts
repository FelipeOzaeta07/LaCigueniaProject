...
import { BASE_URL_PRODUCT, CREATE_PRODUCT, DELETE_PRODUCT, READ_PRODUCT, READ_PRODUCTS, UPDATE_PRODUCT, READ_PRODUCT_ID,  READ_PRODUCTS_RECENTLY_CREATE, READ_PRODUCT_NAME, READ_PRODUCT_CATEGORY  } from "@commons/endpoint/product/ProductEndPoint";
...
export class ProductService extends ProductRepository{
    ...
    override readProductForCategory(categoryId: number): Observable<GenericResponse> {
        return this.http
        .get<GenericResponse>(BASE_URL_PRODUCT + READ_PRODUCT_CATEGORY + categoryId)
        .pipe(catchError((error: HttpErrorResponse) => {
             return throwError(error);
         }));
    }
